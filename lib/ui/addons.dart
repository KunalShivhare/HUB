...
listAddons(List<Widget> list, double windowWidth, Function() _redraw, MainModel _mainModel){
  List<Widget> list2 = [];
  for (var item in _mainModel.currentService.addon)
    list2.add(Container(
        ...
        child: Column(
          children: [
            button2addon(getTextByLocale(item.name), item.selected ? theme.style11W800W : theme.style11W600,
                "\$${item.price.toStringAsFixed(0)}", item.selected ? theme.style11W800W : theme.style11W600,
                //
                item.selected ? mainColor : Colors.grey.withAlpha(20), 10,
                    (){
  ...
  return Row(
    ...
    children: [
      Container(
        ...
        decoration: BoxDecoration(
          color: item.needCount > 1 ? mainColor : Colors.grey,
